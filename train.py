...
model.fit(
    ...
    epochs=configs.train_epochs,  # Set the number of epochs
    steps_per_epoch=10,  # Set the number of steps per epoch
    callbacks=[
        earlystopper,
        ModelCheckpoint(
            f"/Users/aaronshields/Desktop/CSCI-360/Research_project/Models/04_sentence_recognition/Checkpoints/model.keras",
            monitor="val_CER",
            verbose=1,
            save_best_only=True,
            mode="min",
...
# Convert data to pandas DataFrame for training and validation sets
train_df = pd.DataFrame(train_data_provider.dataset, columns=['ImagePath', 'Label'])
val_df = pd.DataFrame(val_data_provider.dataset, columns=['ImagePath', 'Label'])
